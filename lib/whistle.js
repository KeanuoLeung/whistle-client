...
const getShadowRules = (settings) => {
  ...
  const auth =
    username || password ? `* whistle.proxyauth://${username}:${password}` : "";
...
const proxy = startWhistle(
  ...
  () => {
    ...
    try {
      fs.writeFileSync(
        PROC_PATH,
        `${process.pid},${host},${port},${SPECIAL_AUTH}`
      );
    } catch (e) {}
  }
);
proxy.setShadowRules("alo.com www.im-here.cn");
console.log("shadow rules", proxy.getShadowRules());
